...
│   └── myproject/                       # Example: project-specific data
...
```json
{
  "sourceRoot": "C:\\path\\to\\your-repo\\src",
  "scannedUtc": "2026-03-15T10:00:00Z",
  "assemblyPath": "C:\\...\\YourProject.dll",
  "coveragePath": "C:\\...\\coverage.cobertura.xml",
  "testsPath": "C:\\...\\YourProject.Tests"
}
```
...
- **Watch mode** (`scan --watch`): Runs initial scan, then uses `FileSystemWatcher` to monitor assembly/coverage/test files and auto-rescan on changes. Debounces 1.5s. Runs until Ctrl+C.
...
Gotchas are not seeded by the framework — they are written by the agent during sessions via the `add_gotcha` tool and accumulate over time. A gotcha row looks like:

| Type | Category | Gotcha |
|------|----------|--------|
| `MyRecord` | constructor | Parameterless ctor leaves required props null |
| `MyRecord` | equality | partial record — auto-generates `==` value equality |
| `MyEnum` | enum | Default value 0 is `Unknown`, not a real value |
| `IMyService` | mock | `Get(...)` returns `IResult`, not the concrete `Result` |

After a few sessions, `gotchas.jsonl` will contain dozens of project-specific traps that future sessions surface via `get_gotchas` / `get_context`.

### 10.2 Mock Recipes

Mock recipes are auto-generated by the scanner from the test inventory when `--enrich` is passed. Each entry stores Moq setup code patterns extracted from real usage:

```jsonl
{"interfaceName":"IMyService","setupCode":"var mock = new Mock<IMyService>();\nmock.Setup(x => x.Get(It.IsAny<int>())).Returns(...);","usageExamples":["..."]}
```

Recipes for the 10–20 most-used interfaces in your test project are typically enough — agents query them via `get_mock_recipe` when writing new tests.
...
2. Scan assembly + coverage + tests:
   dotnet run --project C:\...\Total.Recall\src\Total.Recall -- scan
     --assembly "C:\...\YourProject\bin\Debug\net8.0\YourProject.dll"
     --coverage "C:\...\TestResults\<guid>\coverage.cobertura.xml"
     --tests "C:\...\YourProject.Tests"
     --output "C:\...\Total.Recall\data\myproject"
...
|------|-----------|--------|------------|
| MetadataLoadContext can't resolve all target assembly dependencies | Medium | Scanner fails | Fallback: use `Assembly.LoadFrom` with all dependency DLLs on the load path |
